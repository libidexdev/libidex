                ...
                foreach ($fieldset['fields'] as $field) {
                    ...
                    if (strlen(trim($value)) && $field_visibility) {
                        if (!in_array($field->getType(), $options['skip_fields']) && $field->getResultDisplay() != 'off') {
							...
							// Display the order ref and not the id
							if ($field_name == "Order Number") {
							
								// is this order ref?
								if (strpos($value, "0100")) {
								
								}
								
								// if not get order from id and set value to increment id
								else {
									if ($_item = Mage::getModel('sales/order_item')->load($value)) {
										$orderId = $_item->getOrderId();
										if ($_order = Mage::getModel('sales/order')->load($orderId)) {
