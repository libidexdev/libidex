...
class Amasty_Shopby_Block_Catalog_Layer_Filter_Attribute extends Amasty_Shopby_Block_Catalog_Layer_Filter_Attribute_Adapter
{
    public function getItemsAsArray()
    {
        ...
        foreach (parent::getItems() as $itemObject){
            ...
            if ($sortBy == 0)
            {
                ...
                {
                    case 'XS':
                        $item['forcedposition'] = 10;
                        break;
                    case 'S':
                        $item['forcedposition'] = 20;
                        break;
                    case 'S/M':
                        $item['forcedposition'] = 30;
                        break;
                    case 'M':
                        $item['forcedposition'] = 40;
                        break;
                    case 'M/L':
                        $item['forcedposition'] = 50;
                        break;
                    case 'L':
                        $item['forcedposition'] = 60;
                        break;
                    case 'L/XL':
                        $item['forcedposition'] = 70;
                        break;
                    case 'XL':
                        $item['forcedposition'] = 80;
                        break;
                    case 'XXL':
                        $item['forcedposition'] = 90;
                        break;
                    case 'ONE SIZE':
                        $item['forcedposition'] = 100;
                        break;
                    default:
                        $item['forcedposition'] = 0;
                        break;
                }
            }
            $items[] = $item;
    ...
    {
        if ($a['forcedposition'] == $b['forcedposition']) {
            return 0;
        }

        return ($a['forcedposition'] > $b['forcedposition'] ? 1 : -1);
    }
