...
/**
 * Magenotification Adminhtml Feedback Edit Tab Form Block
 *
 * @category Magestore
 * @package  Magestore_Magenotification
...
class Magestore_Magenotification_Block_Adminhtml_Feedback_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $this->setForm($form);
        $fieldset = $form->addFieldset('feedback_form', array(
            'legend' => Mage::helper('magenotification')->__('Feedback detail')
        ));
        ...
        if ($dataObject->getFeedbackId()) {
            $fieldset->addField('is_sent', 'note', array(
                'label' => '',
                'name' => 'is_sent',
                'text' => $dataObject->getIsSent() == '1' ?
                    '<span style="width:250px;" class="grid-severity-notice"><span>'
                    . $this->__('Sent to Magestore.com') . '</span></span>' :
                    '<span style="width:250px;" class="grid-severity-critical"><span>'
                    . $this->__('Not sent to Magestore.com') . '</span></span>',
            ));
        }
        ...
        if ($dataObject->getFeedbackId()) {
            ...
            $fieldset->addField('attached_file', 'note', array(
                ...
                'label' => Mage::helper('magenotification')->__('Attached Files'),
                'text' => $this->getLayout()
                    ->createBlock('magenotification/adminhtml_feedback_renderer_file')->toHtml(),
            ));
        }
